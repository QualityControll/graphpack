...
mod tests {
    ...
    #[test] fn tuple_adapters_preserve_alignment_through_take_and_skip(){let x=Input::<i32>::new("x");let y=Input::<i32>::new("y");let graph=x.sequence().zip(y.sequence()).skip(1).take(2).map(|(a,b)|a*10+b).collect();let output:Tensor<i32>=run_graph(&graph,vec![("x",Tensor::new(&[4]).with_values(&[1,2,3,4]).unwrap()),("y",Tensor::new(&[4]).with_values(&[10,20,30,40]).unwrap())]);assert_eq!(output.to_vec(),vec![40,60]);}
