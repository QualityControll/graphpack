...
mod tests {
    ...
    #[test]
    fn tuple_elements_can_be_reused_without_clone() {
        let x = Input::<i32>::new("x");
        let y = Input::<i32>::new("y");
    ...
    #[test]
    fn tuple_elements_can_be_used_multiple_times_with_scalars() {
        let x = Input::<i32>::new("x");
        let y = Input::<i32>::new("y");
        let graph = (x, y).map(|(x, y)| (x + 1) * (x + 2) + (y * 2)).collect();
        let output: Tensor<i32> = run_graph(&graph, vec![("x", Tensor::from(2_i32)), ("y", Tensor::from(3_i32))]);
        assert_eq!(output[0], 19);
    }

    #[test]
    fn eight_tuple_elements_can_be_destructured_and_reused() {
        let inputs = (
        ...
        let graph = inputs.map(|(a,b,c,d,e,f,g,h)| a*a + b*b + c*c + d*d + e*e + f*f + g*g + h*h).collect();
    ...
    fn nested_four_input_tuples_can_be_destructured() {
        ...
        let output:Tensor<i32>=run_graph(&graph,vec![("a",Tensor::from(1_i32)),("b",Tensor::from(2_i32)),("c",Tensor::from(3_i32)),("d",Tensor::from(4_i32)),("e",Tensor::from(5_i32)),("f",Tensor::from(6_i32)),("g",Tensor::from(7_i32)),("h",Tensor::from(8_i32))]);
