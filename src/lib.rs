...
mod tests {
    ...
    #[test] fn enumerate_map_is_composable(){let x=Input::<i32>::new("x");let graph=x.sequence().enumerate().map(|(i,v)|v+i as i32).collect();let output:Tensor<i32>=run_graph(&graph,vec![("x",Tensor::new(&[4]).with_values(&[10,20,30,40]).unwrap())]);assert_eq!(output.to_vec(),vec![10,21,32,43]);}
    ...
    #[test] fn enumerate_can_be_followed_by_regular_sequence_map(){let x=Input::<i32>::new("x");let graph=x.sequence().enumerate().map(|(i,v)|v*2+i as i32).map(|v|v+1).collect();let output:Tensor<i32>=run_graph(&graph,vec![("x",Tensor::new(&[3]).with_values(&[5,6,7]).unwrap())]);assert_eq!(output.to_vec(),vec![11,14,17]);}
