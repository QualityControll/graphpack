...
mod tests {
    ...
    #[test] fn complex64_works_in_map(){let x=Input::<Complex64>::new("x");let graph=x.map(|v|v*Complex::new(2.0_f32,1.0_f32)).collect();let output:Tensor<Complex<f32>>=run_graph(&graph,"x",Tensor::from(Complex::new(1.0_f32,2.0_f32)));assert_eq!(output[0],Complex::new(0.0_f32,5.0_f32));}
    #[test] fn complex128_works_in_map(){let x=Input::<Complex128>::new("x");let graph=x.map(|v|v*Complex::new(2.0_f64,1.0_f64)).collect();let output:Tensor<Complex<f64>>=run_graph(&graph,"x",Tensor::from(Complex::new(1.0_f64,0.0_f64)));assert_eq!(output[0],Complex::new(2.0_f64,1.0_f64));}
    #[test] fn complex64_constants_work_in_map(){let x=Input::<Complex64>::new("x");let graph=x.map(|v|v+Complex::new(1.0_f32,2.0_f32)).collect();let output:Tensor<Complex<f32>>=run_graph(&graph,"x",Tensor::from(Complex::new(3.0_f32,4.0_f32)));assert_eq!(output[0],Complex::new(4.0_f32,6.0_f32));}
    #[test] fn complex128_constants_work_in_map(){let x=Input::<Complex128>::new("x");let graph=x.map(|v|v+Complex::new(1.0_f64,2.0_f64)).collect();let output:Tensor<Complex<f64>>=run_graph(&graph,"x",Tensor::from(Complex::new(3.0_f64,4.0_f64)));assert_eq!(output[0],Complex::new(4.0_f64,6.0_f64));}
    #[test] fn string_comparison_works_in_map(){let x=Input::<String>::new("x");let graph=x.map(|v|v.eq_scalar("hello")).collect();let output:Tensor<bool>=run_graph(&graph,"x",Tensor::from("hello".to_string()));assert!(output[0]);}
    #[test] fn string_inequality_works_in_map(){let x=Input::<String>::new("x");let graph=x.map(|v|v.ne_scalar("hello")).collect();let output:Tensor<bool>=run_graph(&graph,"x",Tensor::from("world".to_string()));assert!(output[0]);}
}
