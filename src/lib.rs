...
mod tests {
    ...
    #[test] fn complex128_works_in_map(){let x=Input::<Complex128>::new("x");let graph=x.map(|v|v*Complex::new(2.0_f64,1.0_f64)).collect();let output:Tensor<Complex<f64>>=run_graph(&graph,"x",Tensor::from(1.0_f64.into()));assert_eq!(output[0],Complex::new(2.0_f64,1.0_f64));}
