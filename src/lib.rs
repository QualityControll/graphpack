...
mod tests {
    ...
    #[test] fn enumerate_returns_index_and_value_sequences(){let x=Input::<i32>::new("x");let (indices, values)=x.sequence().enumerate();let graph=indices.map(|i|i+10).collect();let output:Tensor<i64>=run_graph(&graph,vec![("x",Tensor::new(&[4]).with_values(&[7,8,9,10]).unwrap())]);assert_eq!(output.to_vec(),vec![10,11,12,13]);let graph=values.map(|v|v+1).collect();let output:Tensor<i32>=run_graph(&graph,vec![("x",Tensor::new(&[4]).with_values(&[7,8,9,10]).unwrap())]);assert_eq!(output.to_vec(),vec![8,9,10,11]);}
    #[test] fn zip_returns_left_and_right_sequences(){let x=Input::<i32>::new("x");let y=Input::<i32>::new("y");let (left,right)=x.sequence().zip(y.sequence());let graph=left.map(|v|v+1).collect();let output:Tensor<i32>=run_graph(&graph,vec![("x",Tensor::new(&[3]).with_values(&[1,2,3]).unwrap()),("y",Tensor::new(&[3]).with_values(&[10,20,30]).unwrap())]);assert_eq!(output.to_vec(),vec![2,3,4]);let graph=right.map(|v|v+1).collect();let output:Tensor<i32>=run_graph(&graph,vec![("x",Tensor::new(&[3]).with_values(&[1,2,3]).unwrap()),("y",Tensor::new(&[3]).with_values(&[10,20,30]).unwrap())]);assert_eq!(output.to_vec(),vec![11,21,31]);}
