...
impl<A, B> GraphSeq2<A, B> {
    ...
    where F: FnOnce((GraphValue<A>, GraphValue<B>)) -> GraphValue<U> {
        let value = f((GraphValue::from_node(self.left), GraphValue::from_node(self.right)));
...
impl<T> GraphSeq<T> {
    ...
    pub fn zip<U>(self, other: GraphSeq<U>) -> GraphSeq2<T, U> { let left = crate::graph::insert(Op::new(OpKind::ZipLeft, vec![self.node, other.node])); let right = crate::graph::insert(Op::new(OpKind::ZipRight, vec![self.node, other.node])); GraphSeq2::from_nodes(left, right) }
...
impl GraphSeq<bool> {
    pub fn any(self) -> GraphValue<bool> { GraphValue::from_node(crate::graph::insert(Op::new(OpKind::ReduceAny, vec![self.node]))) }
    pub fn all(self) -> GraphValue<bool> { GraphValue::from_node(crate::graph::insert(Op::new(OpKind::ReduceAll, vec![self.node]))) }
}
