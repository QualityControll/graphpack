...
impl<T:GraphType> Input<T> {
 ...
 pub fn take(self,count:usize)->GraphSeq<T>{self.sequence().take(count)} pub fn skip(self,count:usize)->GraphSeq<T>{self.sequence().skip(count)}
 pub fn sum(self)->GraphValue<T>{self.sequence().sum()} pub fn product(self)->GraphValue<T>{self.sequence().product()} pub fn min(self)->GraphValue<T>{self.sequence().min()} pub fn max(self)->GraphValue<T>{self.sequence().max()} pub fn count(self)->GraphValue<i64>{self.sequence().count()}
...
impl<T> GraphSeq<T>{
 ...
 pub fn enumerate(self)->(GraphSeq<i64>,GraphSeq<T>){let node=crate::graph::insert(Op::new(OpKind::Enumerate,vec![self.node]));(GraphSeq::from_node(crate::graph::insert(Op::new(OpKind::EnumerateIndex,vec![node]))),GraphSeq::from_node(crate::graph::insert(Op::new(OpKind::TupleGet{index:1},vec![node]))))}
 pub fn zip<U>(self,other:GraphSeq<U>)->(GraphSeq<T>,GraphSeq<U>){let node=crate::graph::insert(Op::new(OpKind::Zip,vec![self.node,other.node]));(GraphSeq::from_node(crate::graph::insert(Op::new(OpKind::ZipLeft,vec![node]))),GraphSeq::from_node(crate::graph::insert(OpKind::ZipRight,vec![node])))}
