...
impl<T: GraphType> Input<T> {
        ...
        F: FnOnce(GraphValue<T>) -> GraphValue<bool>,
    {
        GraphSeq::from_node(crate::graph::insert(Op::new(
            OpKind::Filter,
            vec![self.node, predicate(GraphValue::from_node(self.node)).node],
        )))
    }
...
impl<T> GraphSeq<T> {
    ...
    pub fn enumerate(self) -> (GraphSeq<i64>, GraphSeq<T>) {
        let index = crate::graph::insert(Op::new(OpKind::EnumerateIndex, vec![self.node]));
        let value = crate::graph::insert(Op::new(OpKind::TupleGet { index: 1 }, vec![self.node]));
        (GraphSeq::from_node(index), GraphSeq::from_node(value))
    }
    pub fn zip<U>(self, other: GraphSeq<U>) -> (GraphSeq<T>, GraphSeq<U>) {
        ...
        (GraphSeq::from_node(left), GraphSeq::from_node(right))
    }
    pub fn sum(self) -> GraphValue<T> {
