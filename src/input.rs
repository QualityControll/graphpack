...
#[derive(Clone)]
pub struct Input<T> { op: Rc<Op>, _marker: PhantomData<T> }

impl<T: GraphType> Input<T> {
    pub fn new(name: impl Into<String>) -> Self {
        Self { op: Rc::new(Op::new(OpKind::Input { name: name.into(), dtype: T::scalar_type() }, Vec::new())), _marker: PhantomData }
    }
    ...
    where F: FnOnce(GraphValue<T>, GraphValue<T>) -> GraphValue<T> { todo!("reduce graph construction is not implemented yet") }
