...
impl<T: GraphType> Input<T> {
    ...
    pub fn map<U, F>(self, f: F) -> U where F: FnOnce(GraphValue<T>) -> U { f(GraphValue::from_op(self.op)) }
    ...
    ($(($($input:ident, $value:ident, $index:tt),+)),+ $(,)?) => {
        $(
            impl<$($input: GraphType),+> InputTupleMap for ($(Input<$input>,)+) {
                type GraphValues<'a> = ($( &'a GraphValue<$input>, )+) where Self: 'a;
...
impl<A: GraphType, B: GraphType> InputTupleMap for ((Input<A>, Input<B>),) {
    type GraphValues<'a> = ((&'a GraphValue<A>, &'a GraphValue<B>),) where Self: 'a;
    fn map<U, Func>(self, f: Func) -> U where Func: for<'a> FnOnce(Self::GraphValues<'a>) -> U {
        let ((a, b),) = self;
...
impl<A: GraphType, B: GraphType, C: GraphType, D: GraphType> InputTupleMap for ((Input<A>, Input<B>), (Input<C>, Input<D>)) {
    type GraphValues<'a> = ((&'a GraphValue<A>, &'a GraphValue<B>), (&'a GraphValue<C>, &'a GraphValue<D>)) where Self: 'a;
    fn map<U, Func>(self, f: Func) -> U where Func: for<'a> FnOnce(Self::GraphValues<'a>) -> U {
        let ((a, b), (c, d)) = self;
...
impl<A: GraphType, B: GraphType, C: GraphType, D: GraphType, E: GraphType, F: GraphType, G: GraphType, H: GraphType> InputTupleMap for ((Input<A>, Input<B>, Input<C>, Input<D>), (Input<E>, Input<F>, Input<G>, Input<H>)) {
    type GraphValues<'a> = ((&'a GraphValue<A>, &'a GraphValue<B>, &'a GraphValue<C>, &'a GraphValue<D>), (&'a GraphValue<E>, &'a GraphValue<F>, &'a GraphValue<G>, &'a GraphValue<H>)) where Self: 'a;
    fn map<U, Func>(self, func: Func) -> U where Func: for<'a> FnOnce(Self::GraphValues<'a>) -> U {
