...
impl<A, B> GraphSeq2<A, B> {
    ...
    pub fn map<U, F>(self, f: F) -> GraphSeq<U> where F: FnOnce((GraphValue<A>, GraphValue<B>)) -> GraphValue<U> {
        let value = f((GraphValue::from_node(self.left), GraphValue::from_node(self.right)));
        GraphSeq::from_node(value.node)
    }
    pub fn filter<F>(self, predicate: F) -> GraphSeq2<A, B> where F: FnOnce((GraphValue<A>, GraphValue<B>)) -> GraphValue<bool> {
        let p = predicate((GraphValue::from_node(self.left), GraphValue::from_node(self.right)));
