    ...
    for op in graph.operations() {
        ...
        let operation = match op.kind() {
            OpKind::Input { name, dtype } => {
                ...
                desc.set_attr_shape("shape", &Shape::new(Some(vec![])))
                    .map_err(|e| e.to_string())?;
...
fn set_constant(
    ...
    match value {
        ConstantValue::F32(value) => {
            desc.set_attr_type("dtype", DataType::Float).map_err(|e| e.to_string())?;
            desc.set_attr_tensor("value", Tensor::<f32>::from(*value)).map_err(|e| e.to_string())?;
        }
        ConstantValue::F64(value) => {
            desc.set_attr_type("dtype", DataType::Double).map_err(|e| e.to_string())?;
            desc.set_attr_tensor("value", Tensor::<f64>::from(*value)).map_err(|e| e.to_string())?;
        }
        ConstantValue::I32(value) => {
            desc.set_attr_type("dtype", DataType::Int32).map_err(|e| e.to_string())?;
            desc.set_attr_tensor("value", Tensor::<i32>::from(*value)).map_err(|e| e.to_string())?;
        }
        ConstantValue::I64(value) => {
            desc.set_attr_type("dtype", DataType::Int64).map_err(|e| e.to_string())?;
            desc.set_attr_tensor("value", Tensor::<i64>::from(*value)).map_err(|e| e.to_string())?;
        }
        ConstantValue::Bool(value) => {
            desc.set_attr_type("dtype", DataType::Bool).map_err(|e| e.to_string())?;
            desc.set_attr_tensor("value", Tensor::<bool>::from(*value)).map_err(|e| e.to_string())?;
        }
    }
    Ok(())
}
