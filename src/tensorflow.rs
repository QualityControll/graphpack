...
fn lower_operation(tf:&mut TensorFlowGraph,id:NodeId,op:&Op,lowered:&HashMap<NodeId,::tensorflow::Operation>,name:&str)->Result<::tensorflow::Operation,String>{
 ...
 let is_bool_output=matches!(op.kind(),OpKind::Equal|OpKind::NotEqual|OpKind::Less|OpKind::LessEqual|OpKind::Greater|OpKind::GreaterEqual|OpKind::LogicalAnd|OpKind::LogicalOr|OpKind::LogicalNot);
 let mut d=tf.new_operation(typ,name).map_err(|e|e.to_string())?;
 for (index,input) in op.inputs().iter().enumerate(){
  let value=lowered.get(input).ok_or("operation input was not lowered")?.clone();
  if is_u16 && node_scalar_type(*input)==Some(ScalarType::U16) && !matches!(op.kind(),OpKind::LogicalAnd|OpKind::LogicalOr|OpKind::LogicalNot){
   d.add_input(cast(tf,value,DataType::Int32,&format!("{name}_cast_in_{index}"))?);
  } else { d.add_input(value); }
 }
 let operation=d.finish().map_err(|e|e.to_string())?;
 if is_u16 && !is_bool_output {
  return cast(tf,operation,DataType::UInt16,&format!("{name}_u16"));
 }
 Ok(operation)
}

fn cast(tf:&mut TensorFlowGraph,input:::tensorflow::Operation,dtype:DataType,name:&str)->Result<::tensorflow::Operation,String>{let mut d=tf.new_operation("Cast",name).map_err(|e|e.to_string())?;d.add_input(input);d.set_attr_type("DstT",dtype).map_err(|e|e.to_string())?;d.finish().map_err(|e|e.to_string())}
