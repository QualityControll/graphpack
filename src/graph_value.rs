    ...
    ($trait:ident, $method:ident, $kind:expr) => {
        impl<T> $trait for GraphValue<T> {
            ...
            fn $method(self, rhs: Self) -> Self::Output {
                binary_op(self, rhs, $kind)
            }
        }
        impl<T> $trait for &GraphValue<T> {
            type Output = GraphValue<T>;
            fn $method(self, rhs: Self) -> Self::Output {
                binary_op(self.clone(), rhs.clone(), $kind)
            }
        }
        impl<T> $trait<&GraphValue<T>> for GraphValue<T> {
            type Output = GraphValue<T>;
            fn $method(self, rhs: &GraphValue<T>) -> Self::Output {
                binary_op(self, rhs.clone(), $kind)
            }
        }
        impl<T> $trait<GraphValue<T>> for &GraphValue<T> {
            type Output = GraphValue<T>;
            fn $method(self, rhs: GraphValue<T>) -> Self::Output {
                binary_op(self.clone(), rhs, $kind)
            }
        }
    };
}
...
impl<T> Neg for &GraphValue<T> {
    type Output = GraphValue<T>;
    fn neg(self) -> Self::Output {
        unary_op(self.clone(), OpKind::Neg)
    }
}
