...
impl<T> GraphValue<T> {
    ...
    pub fn map<U, F>(&self, f: F) -> U where F: FnOnce(&GraphValue<T>) -> U { f(self) }

    pub fn collect(&self) -> tensorflow::Graph {
        Graph::from_output(self.op.clone()).to_tensorflow().expect("failed to lower GraphPack graph to TensorFlow")
    }
...
pub trait GraphValueTupleMap: Sized {
    ...
    fn map<U, Func>(self, f: Func) -> U where Func: for<'a> FnOnce(Self::GraphValues<'a>) -> U;
}

macro_rules! impl_graph_value_tuple_map {
    ($(($($value:ident),+)),+ $(,)?) => { $(
        impl<$($value),+> GraphValueTupleMap for ($(GraphValue<$value>,)+) {
            type GraphValues<'a> = ($( &'a GraphValue<$value>, )+) where Self: 'a;
            fn map<U, Func>(self, f: Func) -> U where Func: for<'a> FnOnce(Self::GraphValues<'a>) -> U {
                let ($(ref $value, )+) = self;
                f(($( $value, )+))
            }
...
macro_rules! impl_comparison_ops { ($($t:ty),+) => { $(
    impl GraphValue<$t> {
