...
impl_scalar_ops!(f32, f64, i32, i64, bool);
...
mod tests {
    ...
    #[test]
    fn regular_scalar_builds_constant_node() {
        let input = GraphValue::<f32>::from_op(Rc::new(Op::new(
            OpKind::Input { name: "x".into() }, vec![],
        )));
        let result = input * 2.0;

        assert_eq!(result.op().kind(), &OpKind::Mul);
        assert_eq!(result.op().inputs().len(), 2);
        assert_eq!(result.op().inputs()[1].kind(), &OpKind::Constant { value: ConstantValue::F32(2.0) });
    }
}
