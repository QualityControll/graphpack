...
impl LoweringContext {
    pub fn new(scalar_type: ScalarType) -> Self {
        Self { values: HashMap::new(), scalar_type }
    }
        ...
        match expr {
            ...
            Expr::Lit(ExprLit { lit: Lit::Float(value), .. }) => {
                ...
                Ok(quote!({
                    ...
                    constant.set_attr_type("dtype", #self.scalar_type.data_type()).expect("failed to set constant dtype");
                    let value = ::tensorflow::Tensor::<#ty>::new(&[]).with_values(&[#value as #ty]).expect("failed to create constant tensor");
                    constant.set_attr_tensor("value", value).expect("failed to set constant value");
                    constant.finish().expect("failed to finish constant operation")
                }))
            }
            Expr::Binary(ExprBinary { left, op, right, .. }) => {
                ...
                Ok(quote!({
                    ...
                    let mut operation = graph.new_operation(#op_name, "operation").expect("failed to create operation");
...
pub fn input_ident(name: &str) -> Ident { syn::Ident::new(name, proc_macro2::Span::call_site()) }
