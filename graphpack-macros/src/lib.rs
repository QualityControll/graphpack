...
pub fn graphpack(input: TokenStream) -> TokenStream {
    ...
    let (input_name, data_type) = match input {
                ...
                Type::Path(type_path) => {
                    ...
                    match &segment.arguments {
                        PathArguments::AngleBracketed(arguments) if arguments.args.len() == 1 => match arguments.args.first().unwrap() {
                            GenericArgument::Type(Type::Path(type_path)) if type_path.path.is_ident("f32") => quote!(::tensorflow::DataType::Float),
                            other => return syn::Error::new_spanned(other, "graphpack! currently supports Input<f32>").to_compile_error().into(),
                        },
        ...
        _ => return syn::Error::new_spanned(input, "graphpack! inputs must have the form |x: Input<T>| x + 1.0").to_compile_error().into(),
    };

    let body = match closure.body.as_ref() {
        Expr::Binary(ExprBinary { left, op: BinOp::Add(_), right, .. }) => {
            if !matches!(left.as_ref(), Expr::Path(path) if path.path.is_ident(&input_name)) {
                return syn::Error::new_spanned(left, "left operand must be the graph input").to_compile_error().into();
            }
            let value = match right.as_ref() {
                Expr::Lit(syn::ExprLit { lit: syn::Lit::Float(lit), .. }) => lit.clone(),
                _ => return syn::Error::new_spanned(right, "right operand must be an f32 literal").to_compile_error().into(),
            };
            quote! {
        ...
        _ => return syn::Error::new_spanned(&closure.body, "graphpack! currently supports |x: Input<f32>| x + 1.0").to_compile_error().into(),
    };

    TokenStream::from(quote! {
        {
            let mut graph = ::tensorflow::Graph::new();
            let mut input_operation = graph.new_operation("Placeholder", #input_name.to_string()).expect("failed to create input operation");
